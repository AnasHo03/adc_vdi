...
MAX_THRUST = 0.6  #0.6 # 1.4  # Pursuit: 1.2  #Timed trial: 1.4 / 1.0 / 1.4 / Competition : 1.6
...
class LineFollower(Node):
    ...
    def veh_remote_callback(self, msg):
        ...
        self.throttle_pwm =  0 #msg.throttle_pwm
    ...
    def uss_callback(self, msg):
        ...
        self.uss_data_front = msg.uss_range[2:5]
        ...
        print("auto detected" +str(self.auto_detected))
    ...
    def lane_callback(self, msg):
        ...
        if self.emergency_stop == False and self.neutral == False:
            ...
            if abs(self.heading_angle) < 0.01 :
                if self.auto_detected :
                    self.send_ackermann(0.0,0.5)
                    self.auto_detected = False 
                    self.lights("off","off","run")
                else:   
                    self.send_ackermann(0.0,0.5)
            elif abs(self.heading_angle) < 0.05 and abs(self.heading_angle) > 0.01:
                if self.auto_detected :
                    ...
                    self.send_ackermann(0.0,0.4)
            elif abs(self.heading_angle) < 0.3 and abs(self.heading_angle) > 0.05:
                if self.auto_detected :
                    self.send_ackermann(0.0,0.4)
                    self.auto_detected = False 
                    self.lights("off","off","run")
                self.send_ackermann(steering_angle, 0.4)
            else :
