...
# range for parking
range_taken = range(0,50)
range_free_parallel = range(50,80)
range_free_cross = range(80, 120)
range_no_parking = range(120, 3000)

PIT_IN_DISTANCE = 4500
PIT_IN_RIGHT = 900
PIT_IN_LEFT = 400
...
class LineFollower(Node):
    def __init__(self):
        ...
        self.pitin_step = 0
        self.pitin_counter_0 = 0
        self.pitin_counter_1 = 0
        self.pitin_counter_2 = 0
    ...
    def lane_callback(self,msg):
        ...
        if self.dashed:
            self.check_true+=1
        elif self.c_slot_det == True:
            pass
        else:
            self.send_ackermann(0.0, 0.35)
        # if (self.dashed==True) and (self.check_true==1):
        #     self.send_ackermann_halt()
        #     time.sleep(1.0)
        #     self.send_ackermann(0.5,0.0)
        #     time.sleep(1.5)
        #     self.send_ackermann(0.5,0.5)
        #     time.sleep(2.0)    
        #     self.send_ackermann(0.5,0.5)
        #     time.sleep(1.5)
        #     self.parking=True
        if self.emergency_stop == False :
            if not self.parking:
                ...
                if self.pitin_step == 0 :
                    if self.pitin_counter_0 == 0 :
                        self.pitin_counter_0 = 1
                        self.reset_displacement()
                    if abs(self.difference_displcament) <= PIT_IN_DISTANCE : 
                        #self.send_ackermann(steering_angle, self.thrust)
                        self.send_ackermann(steering_angle, 0.6)
                    else :
                        self.pitin_step = 1
                elif self.pitin_step == 1:
                    if self.pitin_counter_1 == 0 :
                        self.pitin_counter_1 = 1
                        self.reset_displacement()
                    if abs(self.difference_displcament) <= PIT_IN_RIGHT : 
                        self.send_ackermann(0.5, 0.8)
                    else :
                        self.pitin_step = 2                    
                elif self.pitin_step == 2:
                    if self.pitin_counter_2 == 0 :
                        self.pitin_counter_2 = 1
                        self.reset_displacement()
                    if abs(self.difference_displcament) <= PIT_IN_LEFT : 
                        self.send_ackermann(-0.5, 0.8)
                    else :
                        self.pitin_step = 3
                else:
                ...
                if self.c_slot_det == False and self.parked == False:
                        # Filter signal
                    self.center_offset = self.filter_signal_offset(self.center_offset)
                    self.average_thrust = self.filter_signal_thrust(self.thrust)
                    self.current_heading = self.heading_angle
                ...
                elif self.c_slot_det == True and self.started == False and self.parked == False :
                    self.get_logger().error("car stops")
                    self.lights("off","right","run")
                    self.get_logger().error("final usrf distance :" + str (self.dist_usrf))
                    self.get_logger().error("final usrb distance :" + str (self.dist_usrb))
                    self.started = True
                elif self.c_slot_det == True and self.started == True and self.parked == False :           
                    self.get_logger().warn("parking started")
                    self.lights("off","right","run")
                    self.cross_parking_manouver()
                    self.lights("off","off","parked")
                elif self.parked == True :
                    self.send_ackermann_halt()
                    self.lights("ON","all","parked")
    ...
    def cross_ausparken(self):
        ...
        if self.ausparking_step == 0:
            ...
            if self.ausparking_step_counter_1 == 0:
                ...
                self.reset_displacement()
            if abs(self.difference_displcament) <= 1400 : 
    ...
    def cross_parking_manouver(self):
        self.get_logger().info("Start Cross Parking ")    
        if self.parking_step == 0:
            ...
            if self.parking_step_counter_2 == 0:
                ...
                self.reset_displacement()
            if abs(self.difference_displcament) <= 750 : 
                self.send_ackermann(0.5,-0.3)
            else :
                self.parking_step = 3
