    ...
    def lane_callback(self,msg):
        ...
        if self.emergency_stop == False:
            if not self.parking:
                ...
                # self.parking = True
                # self.parking_= False
                if self.p_slot_det == False and self.parked == False :
                    # Filter signal
                    self.center_offset = self.filter_signal_offset(self.center_offset)
                    self.average_thrust = self.filter_signal_thrust(self.thrust)
                    self.current_heading = self.heading_angle
                    ...
                    #self.send_ackermann(0.0, -0.6)
                    self.lights("off","off","run")
                    #self.get_logger().warn('Still finding')
                elif self.p_slot_det == True and self.started == False and self.parked == False:
                    
                    #self.send_ackermann_halt()
                    self.get_logger().error("car stops")
                    self.lights("off","all","run")
                    self.get_logger().error("final usrf distance :" + str (self.dist_usrf))
                    self.get_logger().error("final usrb distance :" + str (self.dist_usrb))
                    self.started = True
                elif self.p_slot_det == True and self.started == True and self.parked == False :           
                    
                    self.get_logger().warn("parking started")
                    start_time = time.time()
                    self.lights("off","right","run")
                    self.parallel_parking_manouver()
                elif self.p_slot_det == False and self.parked == True :
                        self.send_ackermann_halt()
                        self.lights("off","all","run")
    ...
    def parallel_parking_manouver(self):
        ...
        if self.parking_step == 0:
            ...
            if self.parking_step_counter_0 == 0 :
                ...
                self.reset_displacement()
            if abs(self.difference_displcament) <= 300 : 
            ...
            if self.parking_step_counter_3 == 0:
                ...
                self.reset_displacement()
            if abs(self.difference_displcament) <= 500 : 
                self.send_ackermann(-0.5,-0.4)
            else :
                self.parking_step = 4
