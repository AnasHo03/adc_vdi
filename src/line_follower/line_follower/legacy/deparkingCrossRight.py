...
class Chatter():
    def __init__(self):
        ...
        self.p_stage = 1
        self.i = 0
        self.deparkingTimer = 0
        self.deparkingStart = 0
        print("deparkingTimer: ",self.deparkingTimer)
        self.counter=0
    ...
    def security_cb(self,times):
        ...
        if np.any(usf < us_thresh_f):
            #if self.deparkingTimer == 0:
            #    self.deparkingTimer = int((time.time() - self.deparkingStart) % 60)
            #self.send_vesc_Ctrl(enableMask, 0, 0, MAX_RAD)
            #print("front obstacle detected:")
            #print("remaining time: ",self.deparkingTimer)
            self.obstacle = True
        else:
            self.obstacle = False
            print("no obstacle")
